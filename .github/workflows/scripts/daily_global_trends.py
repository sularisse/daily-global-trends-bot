...
def fetch_url(url: str, data: bytes | None = None, headers: dict | None = None) -> bytes:
...
def build_prompt(items: list[dict]) -> str:
    today_tw = dt.datetime.now(dt.timezone(dt.timedelta(hours=8))).strftime("%Y-%m-%d")
    source_lines = "\n".join(
    ...
    return textwrap.dedent(
        ...
        End with a section titled exactly: 今日全球趨勢判讀
...
def call_openai(prompt: str) -> str:
    ...
    response = __import__("json").loads(raw.decode("utf-8"))
...
def send_telegram(message: str) -> None:
    ...
    for index, chunk in enumerate(chunks, start=1):
        ...
        result = __import__("json").loads(raw.decode("utf-8"))
